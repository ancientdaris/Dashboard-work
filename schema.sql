...
CREATE TABLE public.designer_projects (
  ...
  CONSTRAINT designer_projects_designer_id_fkey FOREIGN KEY (designer_id) REFERENCES public.interior_designers(id)
);
...
CREATE TABLE public.interior_designers (
  ...
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  CONSTRAINT interior_designers_pkey PRIMARY KEY (id),
  CONSTRAINT interior_designers_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id)
);
...
CREATE TABLE public.look_and_feel (
  id uuid NOT NULL DEFAULT uuid_generate_v4(),
  name text NOT NULL UNIQUE,
  description text,
  icon text,
  ...
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  CONSTRAINT look_and_feel_pkey PRIMARY KEY (id)
);
...
CREATE TABLE public.retailers (
  ...
  company_pan_card text,
  company_pan_card_verified boolean DEFAULT false,
  gst_certificate text,
  gst_certificate_verified boolean DEFAULT false,
  udhyam_aadhar text,
  udhyam_aadhar_verified boolean DEFAULT false,
  gumasta_certificate text,
  gumasta_certificate_verified boolean DEFAULT false,
  aoa_document text,
  aoa_document_verified boolean DEFAULT false,
  moa_document text,
  moa_document_verified boolean DEFAULT false,
  certificate_of_incorporation text,
  certificate_of_incorporation_verified boolean DEFAULT false,
  cin_number text,
  owner_pan_card text,
  owner_pan_card_verified boolean DEFAULT false,
  owner_aadhar_card_front text,
  owner_aadhar_card_front_verified boolean DEFAULT false,
  kyc_status text DEFAULT 'pending'::text,
  kyc_submitted_at timestamp with time zone,
  kyc_verified_at timestamp with time zone,
  kyc_rejection_reason text,
  gst_number text,
  owner_aadhar_card_back text,
  owner_aadhar_card_back_verified boolean DEFAULT false,
  CONSTRAINT retailers_pkey PRIMARY KEY (id),
  CONSTRAINT retailers_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id)
);
...
CREATE TABLE public.wholesalers (
  ...
  CONSTRAINT wholesalers_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id)
);